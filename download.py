...
def hf_download_path(repo_path: str, odir: str, max_try: int = 10):
...
def download_by_hash(filepath_dict: dict, odir: str, hash: str, only_level4: bool):
    ...
    if only_level4: # only download images_4 level data
        download_files = []
        all_files = [x for x in all_files if "nerfstudio" not in x]
        all_files = [x for x in all_files if "distorted" not in x]
        all_files = [x for x in all_files if "images/" not in x]
        all_files = [x for x in all_files if "images_2/" not in x]
        all_files = [x for x in all_files if "images_8/" not in x]
        all_files = [x for x in all_files if "input" not in x]
        for f in all_files:
            path = join(odir, f)
